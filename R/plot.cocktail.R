plot.cocktail=function(x, xvar=c("norm","lambda"),color=FALSE,label=FALSE,...)
{
	beta=x$beta
	lambda=x$lambda
	df=x$df
	...
	if(is.null(type))
	{
		if(color==FALSE) matplot(index,t(beta),lty=1,xlab=xlab,ylab=ylab,type="l",pch=200,
		col=gray.colors(12, start = 0.05, end = 0.7, gamma = 2.2),...)
		else matplot(index,t(beta),lty=1,xlab=xlab,ylab=ylab,type="l",pch=500,...)
	}
